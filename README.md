...
⚠️ Pour l'instant on travaille en `localhost` (dev sur le PC uniquement). Pour joindre l'API/MinIO depuis un device physique (mobile), il faudra soit utiliser l'IP locale de la machine hôte (ex. `http://192.168.x.x:3000`), soit exposer via ngrok — à mettre en place plus tard.
